...
__global__ void saxpy_gpu(float *tabX, float * tabY, int c) {
   // Parametrer l'operation avec threadIdx.x;
    int k = threadIdx.x;
    tabY[k] = tabX[k]*c + tabY[k];
...
{ 
    ...
    CUDA_SAFE_CALL(cudaMemcpy((void*)tabloy_d, (void*) tabloy_h, sizeof(float) * nbelements, cudaMemcpyHostToDevice));
      ...
      /** Lancement du kernel **/
    saxpy_gpu <<<nbblockbygrid, nbthreadbyblock>>> (tablox_d, tabloy_d, A);
